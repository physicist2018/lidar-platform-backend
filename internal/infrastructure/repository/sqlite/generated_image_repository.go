...
func (r *GeneratedImageRepository) Create(ctx context.Context, img *domain.GeneratedImage) error {
	...
	_, err := r.db.ExecContext(ctx,
		`INSERT INTO generated_images (id, experiment_id, file_name, object_path, json_data, wavelength, polarization, channel_type, plot_type, image_type, created_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		img.ID, img.ExperimentID, img.FileName, img.ObjectPath, img.JsonData,
...
func (r *GeneratedImageRepository) FindByParams(ctx context.Context, params ports.GeneratedImageParams) (*domain.GeneratedImage, error) {
	row := r.db.QueryRowContext(ctx,
		`SELECT id, experiment_id, file_name, object_path, json_data, wavelength, polarization, channel_type, plot_type, image_type, created_at
